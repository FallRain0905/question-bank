...
async def openai_embedding(
    ...
    _logger = _logging.getLogger("hyper_rag")
    _logger.info(f"[openai_embedding] base_url={base_url}, model={model}, api_key={'*' + api_key[-6:] if api_key and len(api_key) > 6 else 'EMPTY'}")
    openai_async_client = AsyncOpenAI(
        api_key=api_key or os.environ.get("OPENAI_API_KEY"),
        base_url=base_url,
    )
