...
export async function GET(req: NextRequest) {
  ...
  const vertex_id = req.nextUrl.searchParams.get('vertex_id');
  if (!kb_id || !vertex_id) return NextResponse.json({ error: '缺少参数' }, { status: 400 });
  ...
  try {
    const res = await fetch(`${serviceUrl}/api/vertex-neighbor/${kb_id}/${encodeURIComponent(vertex_id)}`, {
